...
public class DriveWithGripper extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        gripPosition = .5;                  // set arm to half way up.
        ...
        while (opModeIsActive())
        {
            ...
            if (gamepad2.a && armPosition > Servo.MIN_POSITION) armPosition -= .01;

            // move arm up on B button if not already at the highest position.
            if (gamepad2.b && armPosition < Servo.MAX_POSITION) armPosition += .01;

            // open the gripper on X button if not already at most open position.
            if (gamepad2.x && gripPosition < Servo.MAX_POSITION) gripPosition = gripPosition + .01;

            // close the gripper on Y button if not already at the closed position.
            if (gamepad2.y && gripPosition > Servo.MIN_POSITION) gripPosition = gripPosition - .01;
            ...
            //telemetry.addData("grip servo", String.format("position=%.2f  actual=%.2f", gripPosition, gripServo.getPosition()));
