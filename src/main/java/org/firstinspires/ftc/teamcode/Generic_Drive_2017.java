...
public class Generic_Drive_2017 extends LinearOpMode {
  ...
  // The chassis-mounted red/blue alliance switch for autonomous mode.
    ...
    public boolean should_keep_running() {
        if(isStarted() && isStopRequested())
            throw new Mecanum_Wheels_Generic.StopImmediatelyException();
        return true;
    }
    ...
    public void check_beacons_and_push_button(String beacon_name,
                                              Mecanum_Wheels_Generic.AllianceColor color_alliance,
                                              double tape_alpha,
                                              int strafe_away,
                                              int strafe_back) {
        ...
        stop_all_motors();
        Mecanum_Wheels_Generic.AllianceColor near_color = read_beacon_color();

        info(log_prefix + "Near side color: " + near_color);

        // Send the color telemetry data for debugging.
        telemetry.addData(beacon_name, near_color + "/" + Mecanum_Wheels_Generic.AllianceColor.Unknown);
        telemetry.update();
        ...
        stop_all_motors();
        Mecanum_Wheels_Generic.AllianceColor far_color = read_beacon_color();
    ...
    public Mecanum_Wheels_Generic.AllianceColor read_beacon_color() {
    ...
    public void runOpMode() {
        ...
        try {
            ...
            if (t instanceof Mecanum_Wheels_Generic.StopImmediatelyException) {
                info("Stop requested!");
                stop_all_motors();
                return;
            }
